...
class Display(object):
    ...
    def draw_arrow(self, x, y, length, color, angle):
        """Draw an arrow pointing in the specified direction based on an angle.

        Args:
            x (int): Starting X position of the arrow.
            y (int): Starting Y position of the arrow.
            length (int): Length of the arrow.
            color (int): RGB565 color value.
            angle (float): Angle of the arrow in degrees (0 degrees points right).
        """
        # Convert angle to radians
        angle_rad = math.radians(angle)

        # Calculate the end point of the arrow body
        end_x = x + int(length * math.cos(angle_rad))
        end_y = y - int(length * math.sin(angle_rad))  # Subtract because y-coordinates increase downwards

        # Draw the arrow body
        self.draw_line(x, y, end_x, end_y, color)

        # Calculate the arrowhead length (1/3 of the arrow body)
        arrowhead_length = length // 3

        # Calculate the angle for the arrowhead lines
        arrowhead_angle1 = angle_rad - math.pi / 4  # 45 degrees offset from the main line
        arrowhead_angle2 = angle_rad + math.pi / 4  # -45 degrees offset from the main line

        # Calculate the end points of the arrowhead lines
        arrowhead_end_x1 = end_x - int(arrowhead_length * math.cos(arrowhead_angle1))
        arrowhead_end_y1 = end_y + int(arrowhead_length * math.sin(arrowhead_angle1))
        arrowhead_end_x2 = end_x - int(arrowhead_length * math.cos(arrowhead_angle2))
        arrowhead_end_y2 = end_y + int(arrowhead_length * math.sin(arrowhead_angle2))

        # Draw the arrowhead
        self.draw_line(end_x, end_y, arrowhead_end_x1, arrowhead_end_y1, color)
        self.draw_line(end_x, end_y, arrowhead_end_x2, arrowhead_end_y2, color)
